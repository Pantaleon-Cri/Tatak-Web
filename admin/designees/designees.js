  ...
  try {
    ...
    designeeList.forEach(designee => {
      ...
      if (designee.category) {
        categoryName = clubsMap[designee.category] || groupsMap[designee.category] || designee.category;
      }
      ...
      tr.innerHTML = `
        ...
        <td>${categoryName || 'N/A'}</td>  <!-- category -->
        <td>${deptName || 'N/A'}</td>      <!-- department -->
        <td>${officeName || 'N/A'}</td>    <!-- office -->
        <td>${designee.institutionalEmail || ""}</td>
        <td>
          <button class="action-btn edit" data-id="${designee.userID}"><i class="fas fa-edit"></i></button>
          <button class="action-btn delete" data-id="${designee.userID}"><i class="fas fa-trash-alt"></i></button>
        </td>
      `;
